...
## MVP-0.4 Phase 1: Per-device persistent playlist

The first insertion of `MyLastTape.LastTape` in a playback device creates an
AutoDJ playlist and saves a copy under `device:getModData().MyLastTape`.
Later insertions into that same device restore and register the saved list
without scanning accessible media. If no compatible source tracks are found,
the original `NMZomboidTheme2` fallback is saved and used instead.

This phase persists the playlist on the playback device only; it does not yet
copy persistence data onto the newly generated cassette when Tali ejects it.
